...
class CustomPageLayout
  attr_reader :runs, :mediabox

  def initialize(chars, mediabox)
    @mediabox = mediabox
    @runs = group_chars_into_runs(chars)
    @mean_font_size = calculate_mean_font_size
  end

  def calculate_mean_font_size
    return DEFAULT_FONT_SIZE if @runs.nil? || @runs.empty?

    font_sizes = @runs.map(&:font_size).compact
    font_sizes.empty? ? DEFAULT_FONT_SIZE : font_sizes.sum.to_f / font_sizes.size
  end

  # we need to filter duplicate characters which seem to be caused by shadowing
  def group_chars_into_runs(chars)
    return [] if chars.nil? || chars.empty?
  ...
  def self.process(pdf_io, pages = nil)
    ...
    requested_pages.each do |page|
      unless page.nil?
        ...
        mediabox = PDF::Reader::Rectangle.new(0,0,0,0)
        custom_layout = CustomPageLayout.new(text_receiver.characters, mediabox)
        runs = custom_layout.runs
